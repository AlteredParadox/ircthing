...
- `make browser-test` runs the layout suite in `web/browser-test/`: the real `Chat` in a real Chromium against the real stylesheet, asserting the geometry invariants that a DOM-less test cannot see (no horizontal overflow at 360 px, no blank strip below the rendered window while panning back through history with a finger down, no scroll movement when a history page lands mid-gesture). It is deliberately **not** part of `make check` — it needs a ~260 MB browser download, and `check` must stay runnable on a fresh clone — so it runs as its own CI job. Run it locally after touching the virtualized list, the row markup, or the stylesheet.
  - The harness data (`web/test/harness/chat-harness.jsx`) is load-bearing, not decoration. These bugs only appear with a deep buffer (most rows never measured) and realistic wire lines (short messages behind long tags — the estimator error is a ratio). A small buffer or long-message-only generator scores a clean pass against code that blanked half a phone screen. If you change the generator, re-verify the suite still fails against a deliberately broken estimator.
