...
// Buffers and NetworkBuffers below share a projection, spelled out in full in
// each rather than concatenated from fragments (a built query string trips
// every "dynamic SQL" scanner, and these are hot enough to want to read at a
// glance). TestNetworkBuffersMatchesBuffersPerNetwork is the drift guard: it
// asserts the two agree row for row, including the unread and marker values.
//
...
// renders as a system row and never bumps unread. (Non-ACTION CTCP is never
// persisted, and a redacted row keeps its PRIVMSG/NOTICE command, so it counts
// consistently on both sides.) Change one, change the other.
...
func (s *Store) Buffers(ctx context.Context) ([]BufferInfo, error) {
	return s.scanBuffers(s.db.QueryContext(ctx, `
...
// latest-message/marker/unread subqueries are evaluated per surviving row — so
// filtering in SQL rather than in Go keeps a reconnect's cost proportional to
// THAT network's buffers instead of the whole store's. A hostile server that
// can force repeated reconnects would otherwise make each one re-aggregate
// every unrelated network's history.
func (s *Store) NetworkBuffers(ctx context.Context, network string) ([]BufferInfo, error) {
	return s.scanBuffers(s.db.QueryContext(ctx, `
		...
		FROM buffers b
		JOIN networks n ON n.id = b.network_id
		WHERE b.archived = 0 AND n.name = ?
		ORDER BY n.name, b.name`, network))
}
