...
func TestMigrationsIdempotentAndRecorded(t *testing.T) {
	...
	if n != 10 {
		t.Fatalf("schema_migrations rows = %d, want 10", n)
	}
	var name string
	if err := s2.db.QueryRow(`SELECT name FROM schema_migrations WHERE version = 10`).Scan(&name); err != nil {
		t.Fatal(err)
	}
	if name != "0010_fts_rebuild.sql" {
